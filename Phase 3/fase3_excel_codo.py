...
EXCEL_PATH = Path(__file__).resolve().parent / "angulos_codo.xlsx"
...
def draw_pose_without_face(frame, pose_landmarks):
    """
    Dibuja el cuerpo completo, pero sin líneas ni puntos de la cara.
    """
    h, w, _ = frame.shape
...
def initialize_excel(path):
    ...
    Si ya existe, lo abre para seguir agregando datos.
    """
    if path.exists():
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = "AngulosCodo"
        ws.append(["Tiempo (s)", "Angulo_codo (deg)"])
        wb.save(path)
    return wb, ws
...
def main():
    ...
    # Preparar Excel
    wb, ws = initialize_excel(EXCEL_PATH)
    ...
    # Cada cuánto guardar en Excel (10 datos por segundo)
    save_interval = 0.1
    last_save_time = 0.0

    print(f"Guardando datos en: {EXCEL_PATH}")
    ...
    try:
        while True:
            ...
            if result.pose_landmarks:
                ...
                # Puntos para el ángulo del codo
            ...
            # Guardar en Excel cada cierto intervalo
            elapsed_time = time.time() - start_time
            if elbow_angle is not None and (elapsed_time - last_save_time) >= save_interval:
                ws.append([round(elapsed_time, 3), round(elbow_angle, 3)])
            ...
            cv2.putText(
                frame,
                f"Guardado cada: {save_interval:.1f} s",
                (10, 65),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (255, 255, 255),
            ...
            cv2.imshow("Fase 3 - Codo a Excel", frame)

            if cv2.waitKey(1) & 0xFF == ord("q"):
                break

    finally:
        # Guardar Excel al salir
        wb.save(EXCEL_PATH)
        cap.release()
        cv2.destroyAllWindows()
        landmarker.close()
        print(f"Datos guardados correctamente en: {EXCEL_PATH}")
